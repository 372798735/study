...
import { getVideoNote, updateVideoNote } from "@/api/video-notes";
...
const loadVideoNote = async () => {
  const id = route.params.id as string;
  pageLoading.value = true;
  try {
    const response = await getVideoNote(Number(id));
    const videoNote = response.data;
    form.title = videoNote.title;
    form.description = videoNote.description || "";
    form.fileType = videoNote.fileType;
    form.fileUrl = videoNote.fileUrl;
    form.fileName = videoNote.fileName;
    form.fileSize = videoNote.fileSize || 0;
  } catch (error) {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        await updateVideoNote(Number(id), videoNoteData);
